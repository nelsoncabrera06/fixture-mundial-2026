...
// Set de todos los equipos reales (para detectar slots ya resueltos).
...
// o con desempate por penales).
function decisiveResult(r) {
  if (!isFinalResult(r)) return false;
  if (r.homeGoals !== r.awayGoals) return true;
  // Empate en tiempo reglamentario/extra: desempata la tanda de penales.
  return r.penaltyHome != null && r.penaltyAway != null && r.penaltyHome !== r.penaltyAway;
}
...
export function resolveSlot(label, seen = new Set()) {
  // Nombre real ya hardcodeado en knockout.js → confirmado directamente.
  if (ALL_TEAMS.has(label)) return { team: label, status: "confirmed" };
  ...
  const homeWins = r.homeGoals !== r.awayGoals
    ...
    : r.penaltyHome > r.penaltyAway;
  const winnerLabel = homeWins ? m.home : m.away;
...
// Ids de los partidos de semifinal y del 3.º puesto (derivados del cuadro).
const SF_IDS = (ROUNDS.find((r) => r.id === "sf")?.matches || []).map((m) => m.id);
const THIRD_PLACE_ID = ROUNDS.find((r) => r.id === "third")?.matches?.[0]?.id || null;

// Resuelve el PERDEDOR de un partido a { team, status } (para el 3.º puesto,
// cuyos slots son "Perdedor 101"/"Perdedor 102"), o null si no está definido.
function resolveLoser(id) {
  const m = MATCH_BY_ID[id];
  if (!m) return null;
  const r = getResult(m);
  if (!decisiveResult(r)) return null;
  const homeWinsL = r.homeGoals !== r.awayGoals
    ? r.homeGoals > r.awayGoals
    : r.penaltyHome > r.penaltyAway;
  const loserLabel = homeWinsL ? m.away : m.home;
  return resolveSlot(loserLabel);
}

// Anota un paso de la llave para un equipo: rival resuelto + su resultado (W/L)
// y marcador si el partido ya terminó con un ganador (un empate sin definición
// queda como resultado nulo). `loserOf` resuelve rivales tipo "Perdedor N".
function annotateStep(m, teamSlot, loserOf = false) {
  // Comparamos por equipo resuelto, no por string: `teamSlot` puede llegar
  // como placeholder ("1.º Grupo I") mientras que `m.home`/`m.away` ya se
  // "activaron" con el nombre real una vez conocido el cruce (o viceversa).
  const teamName = resolveSlot(teamSlot)?.team || teamSlot;
  const teamSide = resolveSlot(m.home)?.team === teamName ? "home" : "away";
  const oppLabel = teamSide === "home" ? m.away : m.home;
  const opp = loserOf ? resolveLoser(oppLabel.match(/\d+/)?.[0]) : resolveSlot(oppLabel);
  const r = getResult(m);
  let teamResult = null;
  let teamGoals = null;
  let oppGoals = null;
  let teamPenalty = null;
  let oppPenalty = null;
  if (isFinalResult(r)) {
    teamGoals = teamSide === "home" ? r.homeGoals : r.awayGoals;
    oppGoals = teamSide === "home" ? r.awayGoals : r.homeGoals;
    if (teamGoals !== oppGoals) {
      teamResult = teamGoals > oppGoals ? "W" : "L";
    } else if (r.penaltyHome != null && r.penaltyAway != null && r.penaltyHome !== r.penaltyAway) {
      // Empate en el marcador: lo definen los penales.
      teamPenalty = teamSide === "home" ? r.penaltyHome : r.penaltyAway;
      oppPenalty = teamSide === "home" ? r.penaltyAway : r.penaltyHome;
      teamResult = teamPenalty > oppPenalty ? "W" : "L";
    }
  }
  return {
    matchId: m.id,
...
// Camina la llave desde la R32 del equipo: arma el path (cortándolo donde lo
// eliminaron) y devuelve el resultado más profundo alcanzado.
function walkBracket(r32Id, r32SlotLabel) {
  const chain = BRACKET_CHAIN[r32Id] || [];
  const path = [];
  let teamSlot = r32SlotLabel;
  let deepest = { status: "alive" };

  for (const id of chain) {
    const m = MATCH_BY_ID[id];
    const step = annotateStep(m, teamSlot);
    path.push(step);

    if (step.teamResult === "L") {
      // Perdió la semi → todavía juega el partido por el 3.º puesto.
      if (m.roundId === "sf" && THIRD_PLACE_ID) {
        const tp = annotateStep(MATCH_BY_ID[THIRD_PLACE_ID], `Perdedor ${id}`, true);
        path.push(tp);
        deepest =
          tp.teamResult === "W"
            ? { status: "thirdplace" }
      ...
      deepest = {
        status: m.roundId === "final" ? "runnerup" : "out",
        roundId: m.roundId,
        roundName: m.roundName,
        elimOpponent: step.opponent,
      };
      return { path, deepest };
    }

    if (step.teamResult === "W" && m.roundId === "final") {
      deepest = { status: "champion" };
      return { path, deepest };
    }

    // Ganó (avanza) o todavía no se definió (proyección): seguimos. En ambos
    // casos en la ronda siguiente el equipo es "Ganador N".
    teamSlot = `Ganador ${id}`;
  }
  return { path, deepest };
}
...
export function getTeamOutcome(team) {
  ...
  // ¿Cómo salió del grupo? → ranura y partido de R32, o eliminado en grupos.
  let qualifyAs = null;
  let r32Id = null;
  let r32SlotLabel = null;

  if (pos === 0 || pos === 1) {
    qualifyAs = pos === 0 ? "first" : "second";
    r32Id = GROUP_R32[group][qualifyAs];
    r32SlotLabel = `${pos + 1}.º Grupo ${group}`;
  } else if (pos === 2) {
    const proj = projectThirdAssignment();
    const winner =
      proj &&
    ...
    if (winner) {
      qualifyAs = "third";
      r32Id = GROUP_R32[winner].first;
      r32SlotLabel = MATCH_BY_ID[r32Id].away; // la ranura "3.º X/Y/Z"
    }
  }

  if (!qualifyAs) {
    return { status: "groups", group, position: pos + 1 };
  }

  // Recorrer la llave: dónde llegó / dónde lo eliminaron.
  const { path, deepest } = walkBracket(r32Id, r32SlotLabel);
  return { ...deepest, qualifyAs, group, position: pos + 1, path };
}
